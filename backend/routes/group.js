...
// List completed group hikes for a user, newest first
router.get("/history/:userId", async (req, res) => {
  try {
    ...
    const groups = await TrailGroup.find({
      ...
      .sort({ "activeTrail.endTime": -1, updatedAt: -1 })
      .populate("members.userId", "name profileImage email")
      .lean();
...
router.get("/path/:groupId", async (req, res) => {
  try {
    ...
    const trail = group.activeTrail || {};
    const start = trail.startTime ? new Date(trail.startTime) : null;
    const end = trail.endTime ? new Date(trail.endTime) : null;
    if (!start || !end) {
      return res.status(400).json({ error: "Trail timeframe unavailable" });
    }
