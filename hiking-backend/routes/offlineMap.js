...
async function downloadTilesForBounds(mapDoc, token) {
  const baseFolder = path.join(process.cwd(), "offline-maps", mapDoc._id.toString());
  ...
  if (total > MAX_TILES) {
    throw new Error(`Selected area too large: ${total} tiles (limit ${MAX_TILES})`);
  }
...
// POST /offline-map/create
router.post("/offline-map/create", async (req, res) => {
  try {
    const token = process.env.MAPBOX_TOKEN;
    if (!token || token.trim() === "") {
      return res.status(400).json({ error: "MAPBOX_TOKEN missing in server environment" });
    }
    ...
    if (minZoom < 0 || maxZoom > 19 || minZoom > maxZoom) {
      return res.status(400).json({ error: "Invalid zoom range" });
    }

    const mapDoc = await OfflineMap.create({
      userId,
      regionName,
      neLat,
      neLng,
    ...
    // Kick off download (do not block response entirely)
    downloadTilesForBounds(mapDoc, token)
      .then(() => console.log(`✅ Offline map completed: ${mapDoc._id}`))
      .catch(async (err) => {
        console.error("❌ Offline map failed:", err);
        mapDoc.status = "failed";
        mapDoc.error = err.message || String(err);
        await mapDoc.save();
      });

    res.json({ success: true, map: mapDoc });
  } catch (err) {
    console.error("❌ Error creating offline map:", err);
