...
class _WeatherWidgetState extends State<WeatherWidget>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _setupMapProviderListener();
    });
  }

  void _setupMapProviderListener() {
    final mapProvider = Provider.of<MapProvider>(context, listen: false);
    mapProvider.addListener(_onMapProviderChanged);

    // If location is already there, load immediately
    if (mapProvider.currentLocation != null) {
      _onMapProviderChanged();
    } else {
      // If no location yet, we might be loading or offline.
      // We can try to rely on MapProvider's init or just wait.
      _checkConnectivity();
    }
  }
  ...
  // Track last used location to avoid redundant excessive calls if unwanted,
  // but MapProvider usually only notifies on change.
  double? _lastLat;
  double? _lastLon;
  ...
  // Kept for manual retry if needed, essentially just checks provider again
